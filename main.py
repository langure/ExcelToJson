...
def doMain(metadatos_source_file):
    ...
    for documento in json_acumulator:
        ...
        for index, row in df.iterrows():
            ...
            metadato = extract_metadato_excel(row, es_llave)
...
def doMainReadingFromGoogle():
    ...
    for documento in json_acumulator:
        ...
        for index, row in df.iterrows():
           #es_llave = not pd.isna(row[LLAVE])
            es_llave = len(row[LLAVE]) > 0
            metadato = extract_metadato_google(row, es_llave)
            metadatos.append(metadato)
